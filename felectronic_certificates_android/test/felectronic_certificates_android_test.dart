import 'package:felectronic_certificates_android/felectronic_certificates_android.dart';
...
void main() {
  ...
  group('FelectronicCertificatesAndroid', () {
    late FelectronicCertificatesAndroid platform;
    late _MockHostApi mockApi;

    setUp(() {
      platform = FelectronicCertificatesAndroid();
      mockApi = _MockHostApi();
      platform.api = mockApi;
    });

    test('can be registered', () {
      FelectronicCertificatesAndroid.registerWith();
      expect(
        FelectronicCertificatesPlatform.instance,
        isA<FelectronicCertificatesAndroid>(),
      );
    });
    ...
    test('signWithDefaultCertificate passes SHA384RSA', () async {
      mockApi.signResult = Uint8List.fromList([10]);

      await platform.signWithDefaultCertificate(
        Uint8List.fromList([42]),
        algorithm: CertSignAlgorithm.sha384rsa,
      );
      expect(mockApi.lastSignAlgorithm, 'SHA384RSA');
    });
    ...
    test('importCertificate throws CertNotSelectedError', () async {
      ...
      expect(
        ...
        throwsA(isA<CertNotSelectedError>()),
      );
    });
  });
}
