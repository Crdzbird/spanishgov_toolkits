...
void main() {
  ...
  group('$MethodChannelFelectronicCertificates', () {
    ...
    test('getAllCertificates returns list of DeviceCertificate', () async {
    ...
    test('getAllCertificates filters out null entries', () async {
      mockApi.allCertificatesResult = [
        DeviceCertificateMessage(
          serialNumber: 'abc123',
          holderName: 'GARCIA, JUAN',
    ...
    test('getDefaultCertificate returns certificate', () async {
      ...
      expect(result!.serialNumber, 'def456');
      expect(result.alias, 'My Cert');
      expect(result.usages, [CertKeyUsage.authentication]);
    });

    test('getDefaultCertificate returns null when none selected', () async {
      mockApi.defaultCertificateResult = null;
    ...
    test('clearDefaultCertificate completes', () async {
      await platform.clearDefaultCertificate();
      expect(mockApi.clearDefaultCalled, isTrue);
    });

    test('signWithDefaultCertificate passes correct algorithm', () async {
    ...
    test('importCertificate passes data and password', () async {
      await platform.importCertificate(
        Uint8List.fromList([1, 2]),
        password: 'secret',
        alias: 'myAlias',
      );
      expect(mockApi.lastImportPassword, 'secret');
      expect(mockApi.lastImportAlias, 'myAlias');
    });
    ...
    test('importCertificate throws CertAlreadyExistsError', () async {
      mockApi.importError = PlatformException(
      ...
      expect(
        () => platform.importCertificate(Uint8List(1)),
        throwsA(isA<CertAlreadyExistsError>()),
      );
    });

    test('deleteDefaultCertificate completes', () async {
