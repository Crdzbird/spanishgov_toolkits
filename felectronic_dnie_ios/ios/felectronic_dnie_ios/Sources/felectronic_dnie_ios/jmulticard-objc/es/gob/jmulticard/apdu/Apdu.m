...
#error "es/gob/jmulticard/apdu/Apdu must not be compiled with ARC (-fobjc-arc)"
#endif
...
- (IOSByteArray *)getBytes {
  if (apduBytes_ == nil) {
    EsGobJmulticardJmcLogger_warningWithNSString_(@"Se han pedido los octetos de una APDU de cuerpo nulo, se devolvera null");
    return nil;
  }
  IOSByteArray *response = [IOSByteArray arrayWithLength:apduBytes_->size_];
...
- (void)setBytesWithByteArray:(IOSByteArray *)apdu {
  if (apdu == nil) {
    ...
    JreStrongAssign(&apduBytes_, nil);
  }
  else {
    if (apdu->size_ == 0) {
    ...
    JreStrongAssignAndConsume(&apduBytes_, [IOSByteArray newArrayWithLength:apdu->size_]);
...
void EsGobJmulticardApduApdu_init(EsGobJmulticardApduApdu *self) {
  NSObject_init(self);
  JreStrongAssign(&self->apduBytes_, nil);
}
