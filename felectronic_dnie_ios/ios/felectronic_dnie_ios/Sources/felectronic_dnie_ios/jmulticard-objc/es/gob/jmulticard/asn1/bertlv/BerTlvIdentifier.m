...
#error "es/gob/jmulticard/asn1/bertlv/BerTlvIdentifier must not be compiled with ARC (-fobjc-arc)"
#endif
...
- (jint)getTagValue {
  ...
  IOSByteArray *tagBytes = [IOSByteArray arrayWithLength:value_->size_ - 1];
  ...
  return [create_JavaMathBigInteger_initWithByteArray_(tagBytes) intValue];
}
...
- (void)decodeWithJavaIoByteArrayInputStream:(JavaIoByteArrayInputStream *)stream {
  ...
  JreStrongAssignAndConsume(&value_, [IOSByteArray newArrayWithBytes:(jbyte[]){ (jbyte) tlvIdFirstOctet } count:1]);
  jint mask = 31;
  if ((tlvIdFirstOctet & mask) == mask) {
    do {
      ...
      JreStrongAssign(&value_, EsGobJmulticardAsn1BertlvBitManipulationHelper_mergeArraysWithByteArray_withByteArray_(value_, [IOSByteArray arrayWithBytes:(jbyte[]){ (jbyte) tlvIdNextOctet } count:1]));
      if (lastOctet) {
        break;
      }
    }
...
- (NSUInteger)hash {
  return [create_JavaMathBigInteger_initWithByteArray_(value_) intValue];
}
...
- (NSString *)description {
  ...
  JavaLangStringBuilder *buf = create_JavaLangStringBuilder_initWithNSString_(@"[");
  {
    IOSByteArray *a__ = value_;
    jbyte const *b__ = ((IOSByteArray *) nil_chk(a__))->buffer_;
    jbyte const *e__ = b__ + a__->size_;
    while (b__ < e__) {
      jbyte element = *b__++;
      [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([buf appendWithNSString:@"0x"])) appendWithNSString:JavaLangInteger_toHexStringWithInt_(element)])) appendWithChar:' '];
    }
  }
  [buf appendWithChar:']'];
