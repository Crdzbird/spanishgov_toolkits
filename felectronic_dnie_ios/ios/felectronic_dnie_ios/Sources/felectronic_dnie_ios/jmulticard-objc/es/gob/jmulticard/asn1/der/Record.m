...
#error "es/gob/jmulticard/asn1/der/Record must not be compiled with ARC (-fobjc-arc)"
#endif
...
- (EsGobJmulticardAsn1DecoderObject *)getElementAtWithInt:(jint)pos {
  if (pos < 0 || pos >= [((id<JavaUtilList>) nil_chk(elements_)) size]) {
    @throw create_JavaLangIndexOutOfBoundsException_initWithNSString_(JreStrcat("$$", @"No existe un elemento en este registro en el indice ", JavaLangInteger_toStringWithInt_(pos)));
  }
...
- (void)decodeValue {
  if (((IOSByteArray *) nil_chk([self getBytes]))->size_ == 0) {
    @throw create_EsGobJmulticardAsn1Asn1Exception_initWithNSString_(@"El valor del objeto ASN.1 esta vacio");
  }
  jint offset = 0;
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(elementsTypes_))->size_; i++) {
    @try {
      remainingBytes = [IOSByteArray arrayWithLength:((IOSByteArray *) nil_chk([self getBytes]))->size_ - offset];
      ...
      @try {
        tmpDo = [((JavaLangReflectConstructor *) nil_chk([((IOSClass *) nil_chk([((EsGobJmulticardAsn1OptionalDecoderObjectElement *) nil_chk(IOSObjectArray_Get(elementsTypes_, i))) getElementType])) getConstructor:[IOSObjectArray arrayWithLength:0 type:IOSClass_class_()]])) newInstanceWithNSObjectArray:[IOSObjectArray arrayWithLength:0 type:NSObject_class_()]];
      }
      @catch (JavaLangException *e) {
        @throw create_EsGobJmulticardAsn1Asn1Exception_initWithNSString_withJavaLangThrowable_(JreStrcat("$$$$$", @"No se ha podido instanciar un ", [((IOSClass *) nil_chk([((EsGobJmulticardAsn1OptionalDecoderObjectElement *) nil_chk(IOSObjectArray_Get(elementsTypes_, i))) getElementType])) getName], @" en la posicion ", JavaLangInteger_toStringWithInt_(i), @" del registro"), e);
      }
    ...
    @catch (JavaLangException *e) {
      ...
      @throw create_EsGobJmulticardAsn1Asn1Exception_initWithNSString_withJavaLangThrowable_(JreStrcat("$I$", @"Error en el elemento ", i, @" del registro ASN.1"), e);
    }
...
void EsGobJmulticardAsn1DerRecord_initWithEsGobJmulticardAsn1OptionalDecoderObjectElementArray_(EsGobJmulticardAsn1DerRecord *self, IOSObjectArray *types) {
  ...
  if (types == nil || types->size_ == 0) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"Los tipos de los elementos del registro no pueden ser nulos ni vacios");
  }
  JreStrongAssignAndConsume(&self->elementsTypes_, [IOSObjectArray newArrayWithLength:types->size_ type:EsGobJmulticardAsn1OptionalDecoderObjectElement_class_()]);
