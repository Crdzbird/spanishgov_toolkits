...
#error "org/bouncycastle/pqc/crypto/rainbow/Layer must not be compiled with ARC (-fobjc-arc)"
#endif
...
- (IOSObjectArray *)plugInVinegarsWithShortArray:(IOSShortArray *)x {
  jshort tmpMult = 0;
  IOSObjectArray *coeff = [IOSShortArray arrayWithDimensions:2 lengths:(jint[]){ oi_, oi_ + 1 }];
  IOSShortArray *sum = [IOSShortArray arrayWithLength:oi_];
  ...
  for (jint k = 0; k < oi_; k++) {
    for (jint i = vi_; i < viNext_; i++) {
      *IOSShortArray_GetRef(nil_chk(IOSObjectArray_Get(coeff, k)), i - vi_) = OrgBouncycastlePqcCryptoRainbowUtilGF2Field_addElemWithShort_withShort_(IOSShortArray_Get(nil_chk(IOSObjectArray_Get(nil_chk(coeff_gamma_), k)), i), IOSShortArray_Get(nil_chk(IOSObjectArray_Get(coeff, k)), i - vi_));
    }
  }
  for (jint k = 0; k < oi_; k++) {
...
- (void)dealloc {
  RELEASE_(coeff_alpha_);
  RELEASE_(coeff_beta_);
  RELEASE_(coeff_gamma_);
  RELEASE_(coeff_eta_);
  [super dealloc];
}
...
void OrgBouncycastlePqcCryptoRainbowLayer_initWithByte_withByte_withShortArray3_withShortArray3_withShortArray2_withShortArray_(OrgBouncycastlePqcCryptoRainbowLayer *self, jbyte vi, jbyte viNext, IOSObjectArray *coeffAlpha, IOSObjectArray *coeffBeta, IOSObjectArray *coeffGamma, IOSShortArray *coeffEta) {
  ...
  JreStrongAssign(&self->coeff_alpha_, coeffAlpha);
  JreStrongAssign(&self->coeff_beta_, coeffBeta);
  JreStrongAssign(&self->coeff_gamma_, coeffGamma);
  JreStrongAssign(&self->coeff_eta_, coeffEta);
}
...
void OrgBouncycastlePqcCryptoRainbowLayer_initWithInt_withInt_withJavaSecuritySecureRandom_(OrgBouncycastlePqcCryptoRainbowLayer *self, jint vi, jint viNext, JavaSecuritySecureRandom *sr) {
  ...
  self->viNext_ = viNext;
  self->oi_ = viNext - vi;
  JreStrongAssignAndConsume(&self->coeff_alpha_, [IOSShortArray newArrayWithDimensions:3 lengths:(jint[]){ self->oi_, self->oi_, self->vi_ }]);
  JreStrongAssignAndConsume(&self->coeff_beta_, [IOSShortArray newArrayWithDimensions:3 lengths:(jint[]){ self->oi_, self->vi_, self->vi_ }]);
  JreStrongAssignAndConsume(&self->coeff_gamma_, [IOSShortArray newArrayWithDimensions:2 lengths:(jint[]){ self->oi_, self->viNext_ }]);
  JreStrongAssignAndConsume(&self->coeff_eta_, [IOSShortArray newArrayWithLength:self->oi_]);
