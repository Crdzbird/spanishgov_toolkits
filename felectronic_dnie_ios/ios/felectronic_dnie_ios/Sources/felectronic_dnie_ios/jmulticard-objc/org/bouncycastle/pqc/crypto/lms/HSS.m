...
//  source: ./jmulticard/jmulticard-j2obc/java/org/bouncycastle/pqc/crypto/lms/HSS.java
//
...
#error "org/bouncycastle/pqc/crypto/lms/HSS must not be compiled with ARC (-fobjc-arc)"
#endif
...
OrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters *OrgBouncycastlePqcCryptoLmsHSS_generateHSSKeyPairWithOrgBouncycastlePqcCryptoLmsHSSKeyGenerationParameters_(OrgBouncycastlePqcCryptoLmsHSSKeyGenerationParameters *parameters) {
  ...
  IOSObjectArray *keys = [IOSObjectArray arrayWithLength:[((OrgBouncycastlePqcCryptoLmsHSSKeyGenerationParameters *) nil_chk(parameters)) getDepth] type:OrgBouncycastlePqcCryptoLmsLMSPrivateKeyParameters_class_()];
  IOSObjectArray *sig = [IOSObjectArray arrayWithLength:[parameters getDepth] - 1 type:OrgBouncycastlePqcCryptoLmsLMSSignature_class_()];
  IOSByteArray *rootSeed = [IOSByteArray arrayWithLength:32];
  ...
  IOSByteArray *zero = [IOSByteArray arrayWithLength:0];
  jlong hssKeyMaxIndex = 1;
  for (jint t = 0; t < keys->size_; t++) {
    if (t == 0) {
      IOSObjectArray_SetAndConsume(keys, t, new_OrgBouncycastlePqcCryptoLmsLMSPrivateKeyParameters_initWithOrgBouncycastlePqcCryptoLmsLMSigParameters_withOrgBouncycastlePqcCryptoLmsLMOtsParameters_withInt_withByteArray_withInt_withByteArray_([((OrgBouncycastlePqcCryptoLmsLMSParameters *) nil_chk(IOSObjectArray_Get(nil_chk([parameters getLmsParameters]), t))) getLMSigParam], [((OrgBouncycastlePqcCryptoLmsLMSParameters *) nil_chk(IOSObjectArray_Get(nil_chk([parameters getLmsParameters]), t))) getLMOTSParam], 0, I, JreLShift32(1, [((OrgBouncycastlePqcCryptoLmsLMSigParameters *) nil_chk([((OrgBouncycastlePqcCryptoLmsLMSParameters *) nil_chk(IOSObjectArray_Get(nil_chk([parameters getLmsParameters]), t))) getLMSigParam])) getH]), rootSeed));
    }
    else {
      IOSObjectArray_SetAndConsume(keys, t, new_OrgBouncycastlePqcCryptoLmsHSS_PlaceholderLMSPrivateKey_initWithOrgBouncycastlePqcCryptoLmsLMSigParameters_withOrgBouncycastlePqcCryptoLmsLMOtsParameters_withInt_withByteArray_withInt_withByteArray_([((OrgBouncycastlePqcCryptoLmsLMSParameters *) nil_chk(IOSObjectArray_Get(nil_chk([parameters getLmsParameters]), t))) getLMSigParam], [((OrgBouncycastlePqcCryptoLmsLMSParameters *) nil_chk(IOSObjectArray_Get(nil_chk([parameters getLmsParameters]), t))) getLMOTSParam], -1, zero, JreLShift32(1, [((OrgBouncycastlePqcCryptoLmsLMSigParameters *) nil_chk([((OrgBouncycastlePqcCryptoLmsLMSParameters *) nil_chk(IOSObjectArray_Get(nil_chk([parameters getLmsParameters]), t))) getLMSigParam])) getH]), zero));
    }
  ...
  return create_OrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters_initWithInt_withJavaUtilList_withJavaUtilList_withLong_withLong_([parameters getDepth], JavaUtilArrays_asListWithNSObjectArray_(keys), JavaUtilArrays_asListWithNSObjectArray_(sig), 0, hssKeyMaxIndex);
}
...
void OrgBouncycastlePqcCryptoLmsHSS_rangeTestKeysWithOrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters_(OrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters *keyPair) {
  ...
  @synchronized(keyPair) {
    if ([((OrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters *) nil_chk(keyPair)) getIndex] >= [keyPair getIndexLimit]) {
      @throw create_OrgBouncycastlePqcCryptoExhaustedPrivateKeyException_initWithNSString_(JreStrcat("$$$", @"hss private key", (([keyPair isShard]) ? @" shard" : @""), @" is exhausted"));
    }
    jint L = [keyPair getL];
    jint d = L;
    id<JavaUtilList> prv = JreRetainedLocalValue([keyPair getKeys]);
    while ([((OrgBouncycastlePqcCryptoLmsLMSPrivateKeyParameters *) nil_chk([((id<JavaUtilList>) nil_chk(prv)) getWithInt:d - 1])) getIndex] == JreLShift32(1, ([((OrgBouncycastlePqcCryptoLmsLMSigParameters *) nil_chk([((OrgBouncycastlePqcCryptoLmsLMSPrivateKeyParameters *) nil_chk([prv getWithInt:d - 1])) getSigParameters])) getH]))) {
      d = d - 1;
      if (d == 0) {
        @throw create_OrgBouncycastlePqcCryptoExhaustedPrivateKeyException_initWithNSString_(JreStrcat("$$$", @"hss private key", (([keyPair isShard]) ? @" shard" : @""), @" is exhausted the maximum limit for this HSS private key"));
      }
    }
    while (d < L) {
...
OrgBouncycastlePqcCryptoLmsHSSSignature *OrgBouncycastlePqcCryptoLmsHSS_generateSignatureWithOrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters_withByteArray_(OrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters *keyPair, IOSByteArray *message) {
  ...
  @synchronized(keyPair) {
    OrgBouncycastlePqcCryptoLmsHSS_rangeTestKeysWithOrgBouncycastlePqcCryptoLmsHSSPrivateKeyParameters_(keyPair);
    id<JavaUtilList> keys = JreRetainedLocalValue([keyPair getKeys]);
    id<JavaUtilList> sig = JreRetainedLocalValue([keyPair getSig]);
  ...
  OrgBouncycastlePqcCryptoLmsLMSContext *context = JreRetainedLocalValue([((OrgBouncycastlePqcCryptoLmsLMSContext *) nil_chk([((OrgBouncycastlePqcCryptoLmsLMSPrivateKeyParameters *) nil_chk(nextKey)) generateLMSContext])) withSignedPublicKeysWithOrgBouncycastlePqcCryptoLmsLMSSignedPubKeyArray:signed_pub_key]);
...
jboolean OrgBouncycastlePqcCryptoLmsHSS_verifySignatureWithOrgBouncycastlePqcCryptoLmsHSSPublicKeyParameters_withOrgBouncycastlePqcCryptoLmsHSSSignature_withByteArray_(OrgBouncycastlePqcCryptoLmsHSSPublicKeyParameters *publicKey, OrgBouncycastlePqcCryptoLmsHSSSignature *signature, IOSByteArray *message) {
  ...
  IOSObjectArray *sigList = [IOSObjectArray arrayWithLength:Nspk + 1 type:OrgBouncycastlePqcCryptoLmsLMSSignature_class_()];
  IOSObjectArray *pubList = [IOSObjectArray arrayWithLength:Nspk type:OrgBouncycastlePqcCryptoLmsLMSPublicKeyParameters_class_()];
  for (jint i = 0; i < Nspk; i++) {
    IOSObjectArray_Set(sigList, i, [((OrgBouncycastlePqcCryptoLmsLMSSignedPubKey *) nil_chk(IOSObjectArray_Get(nil_chk([signature getSignedPubKey]), i))) getSignature]);
    IOSObjectArray_Set(pubList, i, [((OrgBouncycastlePqcCryptoLmsLMSSignedPubKey *) nil_chk(IOSObjectArray_Get(nil_chk([signature getSignedPubKey]), i))) getPublicKey]);
  }
  IOSObjectArray_Set(sigList, Nspk, [signature getSignature]);
  OrgBouncycastlePqcCryptoLmsLMSPublicKeyParameters *key = JreRetainedLocalValue([publicKey getLMSPublicKey]);
  for (jint i = 0; i < Nspk; i++) {
    ...
    @catch (JavaLangException *ex) {
      @throw create_JavaLangIllegalStateException_initWithNSString_withJavaLangThrowable_([ex getMessage], ex);
    }
  }
...
- (OrgBouncycastlePqcCryptoLmsLMOtsPrivateKey *)getNextOtsPrivateKey {
  @throw create_JavaLangRuntimeException_initWithNSString_(@"placeholder only");
}
