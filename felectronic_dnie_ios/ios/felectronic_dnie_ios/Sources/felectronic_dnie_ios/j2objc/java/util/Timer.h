...
/*!
 ...
 <p>Corresponding to each <code>Timer</code> object is a single background
  ...
  completes. 
 <p>After the last live reference to a <code>Timer</code> object goes away 
 <i>and</i> all outstanding tasks have completed execution, the timer's task
  ...
  should invoke the timer's <code>cancel</code> method. 
 <p>If the timer's task execution thread terminates unexpectedly, for
  example, because its <code>stop</code> method is invoked, any further
  attempt to schedule a task on the timer will result in an 
 <code>IllegalStateException</code>, as if the timer's <code>cancel</code>
  method had been invoked. 
 <p>This class is thread-safe: multiple threads can share a single 
 <code>Timer</code> object without the need for external synchronization. 
 <p>This class does <i>not</i> offer real-time guarantees: it schedules
  tasks using the <code>Object.wait(long)</code> method. 
...
/*!
 ...
 @throw IllegalArgumentExceptionif <code>time.getTime()</code> is negative.
...
/*!
 ...
 @throw IllegalArgumentExceptionif <code>delay</code> is negative, or
