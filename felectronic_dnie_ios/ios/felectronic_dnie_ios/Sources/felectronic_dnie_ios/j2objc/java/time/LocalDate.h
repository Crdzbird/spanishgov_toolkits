...
/*!
 @brief Returns a sequential ordered stream of dates.The returned stream starts from this date
  (inclusive) and goes to <code>endExclusive</code> (exclusive) by an incremental step of 1 day.
 <p>
  This method is equivalent to <code>datesUntil(endExclusive, Period.ofDays(1))</code>.
 @param endExclusive the end date, exclusive, not null
 @return a sequential <code>Stream</code> for the range of <code>LocalDate</code> values
 @throw IllegalArgumentExceptionif end date is before this date
 @since 9
 */
- (id<JavaUtilStreamStream>)datesUntilWithJavaTimeLocalDate:(JavaTimeLocalDate *)endExclusive;

/*!
 @brief Returns a sequential ordered stream of dates by given incremental step.The returned stream
  starts from this date (inclusive) and goes to <code>endExclusive</code> (exclusive).
 <p>
  The n-th date which appears in the stream is equal to <code>this.plus(step.multipliedBy(n))</code>
  (but the result of step multiplication never overflows). For example, if this date is 
 <code>2015-01-31</code>, the end date is <code>2015-05-01</code> and the step is 1 month, then the
  stream contains <code>2015-01-31</code>, <code>2015-02-28</code>, <code>2015-03-31</code>, and 
 <code>2015-04-30</code>.
 @param endExclusive the end date, exclusive, not null
 @param step the non-zero, non-negative <code>Period</code>  which represents the step.
 @return a sequential <code>Stream</code> for the range of <code>LocalDate</code> values
 @throw IllegalArgumentExceptionif step is zero, or <code>step.getDays()</code> and
              <code>step.toTotalMonths()</code> have opposite sign, or end date is before this date
              and step is positive, or end date is after this date and step is negative
 @since 9
 */
- (id<JavaUtilStreamStream>)datesUntilWithJavaTimeLocalDate:(JavaTimeLocalDate *)endExclusive
                                         withJavaTimePeriod:(JavaTimePeriod *)step;
...
/*!
 ...
 <p>
  ...
  which are too large to fit in an <code>int</code> and throw an <code>UnsupportedTemporalTypeException</code>.
...
/*!
 @brief Obtains an instance of <code>LocalDate</code> from an <code>Instant</code> and zone ID.
 <p>
 ...
 @since 9
 */
+ (JavaTimeLocalDate *)ofInstantWithJavaTimeInstant:(JavaTimeInstant *)instant
...
/*!
 @brief Converts this <code>LocalDate</code> to the number of seconds since the epoch
 ...
 @since 9
 */
- (jlong)toEpochSecondWithJavaTimeLocalTime:(JavaTimeLocalTime *)time
...
/*!
 @brief The epoch year <code>LocalDate</code>, '1970-01-01'.
 */
inline JavaTimeLocalDate *JavaTimeLocalDate_get_EPOCH(void);
/*! INTERNAL ONLY - Use accessor function from above. */
FOUNDATION_EXPORT JavaTimeLocalDate *JavaTimeLocalDate_EPOCH;
J2OBJC_STATIC_FIELD_OBJ_FINAL(JavaTimeLocalDate, EPOCH, JavaTimeLocalDate *)
...
FOUNDATION_EXPORT JavaTimeLocalDate *JavaTimeLocalDate_ofInstantWithJavaTimeInstant_withJavaTimeZoneId_(JavaTimeInstant *instant, JavaTimeZoneId *zone);
