...
/*!
 @brief Obtains a clock that returns the current instant ticking in whole milliseconds
 ...
 <p>
  The returned implementation is immutable, thread-safe and <code>Serializable</code>.
  It is equivalent to <code>tick(system(zone), Duration.ofMillis(1))</code>.
 @param zone the time-zone to use to convert the instant to date-time, not null
 @return a clock that ticks in whole milliseconds using the specified zone, not null
 @since 9
 */
+ (JavaTimeClock *)tickMillisWithJavaTimeZoneId:(JavaTimeZoneId *)zone;
...
FOUNDATION_EXPORT JavaTimeClock *JavaTimeClock_tickMillisWithJavaTimeZoneId_(JavaTimeZoneId *zone);
...
J2OBJC_STATIC_INIT(JavaTimeClock_SystemClock)

inline JavaTimeClock_SystemClock *JavaTimeClock_SystemClock_get_UTC(void);
/*! INTERNAL ONLY - Use accessor function from above. */
FOUNDATION_EXPORT JavaTimeClock_SystemClock *JavaTimeClock_SystemClock_UTC;
J2OBJC_STATIC_FIELD_OBJ_FINAL(JavaTimeClock_SystemClock, UTC, JavaTimeClock_SystemClock *)
